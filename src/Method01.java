...
//        사용자가 원하는 결과를 얻기 위한 소스코드를 한번 입력할 때는 문제가 없지만 여러번 사용 할 경우
//        소스코드의 중복이 발생

//        매개변수 : 해당 메서드 내의 연산시 필요한 데이터를 외부에서 전달 하는 변수
//        변환값 :해당 메서드의 연산 후 그 결과값을 메서드를 호출한 위치로 다시 되돌려주는 것
//
//
//        메서드 사용법
//        선언 :
...
//          연산 후 결과를 메서드 외부로 되돌려줌
//          연산 결과를 다른곳에서 활용 할 수 있음
//          연산의 결과는 동일함

//        4. 매개변수와 반환값이 모두존재하는 형태
//          연산에 필요한 데이터를 메서드 외부에서 전달 받음
//          연산 후 그 결과를 메서드 외부로 되돌려줌
//          연산 결과를 다른 곳에서 활용할수 있음
//         연산의 결과가 입력되는 데이터따라 변경됨
        ...
        System.out.println("두 수의 곱세은 : " + result);
...
//    매개변수는 존재하고 반환값이 없는 타입
    public static void sumType2(int a, int b){
        int reuslt = a - b;
        System.out.println("두수의 뻴셈은 : " + reuslt);

    }
//    매개변수는 없고 반환값이 존재하는 타입
//    retult 키워드 사용시 반환되는 값의 데이터 타입은 해당 메서드의 반환
//    타입과 일치해야 함
